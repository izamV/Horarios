...
export const ScheduleEditor: FC<Props> = ({ ownerId, ownerRol, ownerNombre }) => {
  ...
  const handleSessionChange = (session: Session, field: keyof Session, value: string | undefined) => {
  ...
  const handleAddMaterial = (session: Session) => {
    if (project.materiales.length === 0) {
    ...
    const materials = session.materiales ? [...session.materiales] : [];
    const defaultMaterial = project.materiales[0];
    materials.push({ materialId: defaultMaterial?.id ?? '', cantidad: 1 });
    setSessionMaterials(session.id, materials);
  };
